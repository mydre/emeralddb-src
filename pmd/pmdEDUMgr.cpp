...
{
   _setDestroyed ( true ) ;
   setQuiesced ( true ) ;//所以，这时不会再有新的EDU进来

   //stop all user edus
   unsigned int timeCounter = 0 ;
   unsigned int eduCount = _getEDUCount ( EDU_USER ) ;//用户EDU

   while ( eduCount != 0 )//如果用户UDU数量不是0
   {
      if ( 0 == timeCounter % 50 )
      {
         _forceEDUs ( EDU_USER ) ;
      ...
      ++timeCounter ;
      ossSleepmillis ( 100 ) ;
      eduCount = _getEDUCount ( EDU_USER ) ;//每0.1秒检测还有没有用户EDU存在
   }
...
int pmdEDUMgr::_forceEDUs ( int property )
{
   ...
   for ( it = _idleQueue.begin () ; it != _idleQueue.end () ; ++it )//空闲队列
   {
...
int pmdEDUMgr::startEDU ( EDU_TYPES type, void* arg, EDUID *eduid )
{
   ...
   if ( true == _idleQueue.empty () || !isPoolable ( type ) )//是listener？
   {
...
int pmdEDUMgr::_createNewEDU ( EDU_TYPES type, void* arg, EDUID *eduid )
{
   ...
   if ( !getEntryFuncByType ( type ) )//获取指针函数？
   {
   ...
   try
   {
	   //PD_LOG(PDEVENT,"status:%d",cb->getStatus());   0
      boost::thread agentThread ( pmdEDUEntryPoint,type, cb, arg ) ;//创建一个新的线程，第一个参数是线程对应的临界区，后面三个参数是线程的参数
      // detach the agent so that he's all on his own
      // we only track based on CB
      agentThread.detach () ;//自己运行结束才释放资源
   }
   ...
   cb->postEvent( pmdEDUEvent( PMD_EDU_EVENT_RESUME, false, arg ) ) ;//激活一个事件，把参数发进去
...
int pmdEDUMgr::waitEDU ( EDUID eduID )
{
   ...
   eduStatus = eduCB->getStatus () ;
   //PD_LOG(PDEVENT,"edustatus:%d",eduStatus);
   ...
   eduCB->setStatus ( PMD_EDU_WAITING ) ;//设置为等待状态
done :
   _mutex.release_shared () ;
   /************* END CRITICAL SECTION **************/
   return rc ;
...
int pmdEDUMgr::_deactivateEDU ( EDUID eduID )//把线程从run queue里面放到idle queue里面
{
   int rc         = EDB_OK ;
   unsigned int eduStatus = PMD_EDU_CREATING ;
   pmdEDUCB* eduCB  = NULL ;
   ...
   if ( _runQueue.end () == ( it = _runQueue.find ( eduID )) )
   {
      ...
      // we can't find EDU in run queue and idle queue
      rc = EDB_SYS ;
      goto error ;
   }
   //in run queue
...
int pmdEDUMgr::activateEDU ( EDUID eduID )//extract from idle queue
{
   int   rc        = EDB_OK ;
   unsigned int  eduStatus = PMD_EDU_CREATING ;
   pmdEDUCB* eduCB   = NULL;
...
{
   _mutex.get() ;
   _tid_eduid_map [ tid ] = eduid ;//associate tid with eduid
   _mutex.release () ;
}
